...
export function parseMoneyInput(text: string): number {
  ...
  if (!/^(?:\d+|\d+[.,]\d{1,2}|\d{1,3}(?:,\d{3})+(?:\.\d{1,2})?|\d{1,3}(?:\.\d{3})+(?:,\d{1,2})?)$/.test(value)) throw new Error('Ingresa un monto válido y positivo.');
  let canonical = value;
  if (value.includes(',') && value.includes('.')) {
    canonical = value.lastIndexOf(',') > value.lastIndexOf('.') ? value.replace(/\./g, '').replace(',', '.') : value.replace(/,/g, '');
  } else if (/^\d{1,3}(?:[.,]\d{3})+$/.test(value)) canonical = value.replace(/[.,]/g, '');
  else canonical = value.replace(',', '.');
  const result = Number(canonical);
  if (!Number.isFinite(result) || result <= 0 || result > Number.MAX_SAFE_INTEGER / 100) throw new Error('Ingresa un monto válido y positivo.');
  return result;
}

function validDate(value: string): string {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value) || !Number.isFinite(Date.parse(value)) || new Date(value).toISOString().slice(0, 10) !== value) throw new Error('La fecha no es válida.');
  return value;
}

export function parseTransactionText(text: string, options: TransactionTextOptions) {
  if (typeof text !== 'string' || !text.trim()) throw new Error('Escribe el detalle y monto de la transacción.');
  ...
  if (relativeDates.length > 1 || (relativeDates.length && (isoDate || namedDate || explicitDate))) throw new Error('Indica una única fecha para la transacción.');
  if (isoDate) {
    ...
    const offset = ({ anteayer: -2, ayer: -1, hoy: 0, manana: 1 } as Record<string, number>)[relativeDates[0]!];
  ...
  if (/(?:^|[^\d])[.,]\d|\d\s*(?:mil|millon|millones|k)\b/.test(amountText)) throw new Error('Escribe el monto completo, por ejemplo 0.50 o 35000.');
  if (/[-−]\s*\d/.test(amountText)) throw new Error('Ingresa un monto válido y positivo.');
  const amounts = amountText.match(/\d+(?:[.,]\d+)*/g) ?? [];
  if (amounts.length !== 1) throw new Error('Indica un único monto para esta transacción.');
  const amount = parseMoneyInput(amounts[0]);
  const type: 'income' | 'expense' = /\b(sueldo|salario|depositaron|pagaron|ingreso|recibi|cobre)\b/.test(clean) && !/\b(pague|gaste|compre)\b/.test(clean) ? 'income' : 'expense';
  ...
  if (currencyHints.length > 1 || /\bpesos?\b/.test(clean) || (clean.includes('$') && !/us\$|\busd\b/.test(clean))) throw new Error('Indica una moneda inequívoca, como PEN, USD o EUR.');
  ...
  if (!account) warnings.push('Selecciona la cuenta: no se identificó una cuenta única.');
  const families = [
  ...
  if (!categoryId) warnings.push('Selecciona la categoría: no se identificó una categoría única.');
...
export function createFinancialReply(prompt: string, context: FinancialReplyContext): string {
  const query = normalize(prompt);
  const money = (value: number) => `${context.currency === undefined || context.currency === 'PEN' ? 'S/' : context.currency} ${value.toFixed(2)}`;
  const period = context.period ?? 'este mes';
  if (/ahorr|disponible/.test(query)) return `La diferencia entre ingresos y gastos de ${period} es ${money(context.monthlyIncome - context.monthlyExpense)}. Es el saldo del período, no una predicción de ahorro.`;
  ...
  if (/resumen|finanza|balance|saldo/.test(query)) return `Saldo total: ${money(context.totalBalance)}. En ${period}: ingresos de ${money(context.monthlyIncome)} y gastos de ${money(context.monthlyExpense)}.`;
  if (/gast/.test(query)) return `Tus gastos registrados de ${period} suman ${money(context.monthlyExpense)}.`;
  if (/ingreso/.test(query)) return `Tus ingresos registrados de ${period} suman ${money(context.monthlyIncome)}.`;
